...
LoadWidget.prototype = {
    ...
    _load: function(){
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if(isPaging){
                    ...
                    backData.set(PAGE, data.page[PAGE]);
                    backData.set(PAGE_COUNT, data.page[PAGE_COUNT]);
                    event.trigger('backData.' + _eventId, backData);
                }
            },
            error: function() {
                _reqError();
        ...
        function _generateHtml(data){
            if (!data[ROWS] || data[ROWS].length <= 0) event.trigger('noData.' + _eventId);
            ...
            event.trigger('hasData.' + _eventId, data, html);
        }

        function _reqError(){
            event.trigger('fail.' + _eventId);
        }
    },
    ...
    render: function(html){
        ...
        event.one("hasData." + _eventId, function(data, html){
            removeStatus();
            content.html(html);
            callback && callback.call(null, true , data);
        });
        event.one("noData." + _eventId, function(){
            removeStatus();
            var noListStr = self.config.get('noListStr');
            status.noData(content, noListStr);
            callback && callback.call(null, false);
        });
        event.one("fail." + _eventId, function(){
            removeStatus();
